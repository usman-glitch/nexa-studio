...
export default function Testimonials() {
  return (
    /* Added scroll-mt-32 to ensure there is breathing room when scrolling from the footer */
    <section id="testimonials" className="relative py-32 bg-white overflow-hidden scroll-mt-32">
      <div className="max-w-[1500px] mx-auto px-6">
        ...
        {/* OPTIONAL: SIMPLE LOGO CLOUD FOR POLISH */}
        <motion.div 
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          className="mt-32 pt-12 border-t border-gray-100 flex flex-wrap justify-center md:justify-between items-center gap-12 opacity-40 grayscale"
          aria-label="Partner brands"
        >
          {["TECHFLOW", "BLOOM AI", "VELO", "ROSSI GROUP", "NEXA CORE"].map((logo) => (
            <span key={logo} className="font-black tracking-[0.5em] text-xs text-gray-400">
              {logo}
            </span>
          ))}
        </motion.div>
